...
<form id="step2_form" action="reservation_step2.do">
	<div>
		...
		<div class="step2_optionBottom">
			<ul>
				<li>
					<input type="checkbox" id="res_option_1" name="res_option_1" value="Daily Breakfast Buffet for Adult">Daily Breakfast Buffet for Adult<br>
					<span>30,000 원</span>
				...
				<li>
					<input type="checkbox" id="res_option_2" name="res_option_2" value="Daily Extra bed">Daily Extra bed<br>
					<span>30,000 원</span>
				</li>
	...
	<!-- 회원 or 비회원의 정보 입력 -->
		  	...
		  	<ul>
		  		<li class="guest_info">
		  			<ul>
			  			...
			  			<li>*성<input type="text" id="pay_lastname" name="pay_lastname" 
			  				value=<c:if test="${!empty user_id}">"${command.mem_lastname}"</c:if>>
			  			</li>
			  			
			  			<li>*이름<input type="text" id="pay_firstname" name="pay_firstname" 
			  				value=<c:if test="${!empty user_id}">"${command.mem_firstname}"</c:if>>
			  			</li>
			  			
			  			<li>*이메일 <input type="text" id="pay_email" name="pay_email"
			  			 value=<c:if test="${!empty user_id}">"${command.mem_email}"</c:if>>
			  			 </li>
			  			 
			  			<li>*연락처(자택) <input type="text" id="pay_h_cell" name="pay_h_cell" onkeydown="onlyNumber(this)" maxlength="11" 
			  			value=<c:if test="${!empty user_id}">"${command.mem_h_cell}"</c:if>>			  			
			  			</li>
			  			
			  			<li>*연락처(휴대전화) <input type="text" id="pay_p_cell" name="pay_p_cell" onkeydown="onlyNumber(this)" maxlength="11" 
		  		...
		  		<li class="payment_info">
		  			<ul>
						...
						<li>*카드종류
							<select id="pay_cardtype" name="pay_cardtype">
								<option></option>
						...
						<li>*카드 번호
							<input type="text" id="pay_card_num1" name="pay_card_num1" size="3" onkeydown="onlyNumber(this)" maxlength="4">-
							<input type="text" id="pay_card_num2" name="pay_card_num2" size="3" onkeydown="onlyNumber(this)" maxlength="4">-
							<input type="text" id="pay_card_num3" name="pay_card_num3" size="3" onkeydown="onlyNumber(this)" maxlength="4">-
							<input type="text" id="pay_card_num4" name="pay_card_num4" size="3" onkeydown="onlyNumber(this)" maxlength="4">
							<input type="hidden" id="pay_cardnum" name="pay_cardnum">
						</li>
						
						<li>*만기일
							<select id="pay_card_mon" name="pay_card_mon">
								<option></option>
							...
							<select id="pay_card_year" name="pay_card_year">
								<option></option>
						...
						<li>
						※ 신용카드 정보는 고객님의 투숙에 의한 개런티 외에는 어떤 목적으로도 사용되지 않습니다. 온라인 예약 시 직접 결제가 이루어지지 않으며,
						최종 결제는 체크아웃 시 프런트 데스크에서 해주시기 바랍니다.
						</li>
		...
		<div class="info_agree">
			<h4>필수적인 개인정보 수집이용에 동의하십니까?</h4>
			<div>
				<div class="info_agree_text1">
					
<textarea rows="5" cols="50" id="agree_area1">
...
<textarea rows="5" cols="50" id="agree_area2">
&lt;개인정보 제3자 제공에 대한 동의&gt;
1. 제공받는 자 | (주)NGU스테이
...
우 NGU 회원가입 및 서비스 제공이 불가능함을 알려드립니다.
		...
		<div>
			<div class="trans_btn">
				<span class="step_back"><input type="button" onclick="location.href='reservation.do'" value="뒤로가기"></span>
				
				<span class="rsv_success"><input type="submit" value="예약신청"></span>
			</div>
		</div>
	...
	<div class="step_right">
		...
		<h4>예약 정보</h4>
		<div>
				<input type="button" value="예약초기화" onclick="location.href='reservation.do'">
			<ul>	
				<li>	
					호텔: 
					<span>N.G.U 호텔</span>
		...
		<div>
			<ul>
				<li>체크인 : <span id="in_day">${reservationCommand.res_in}</span></li>
				...
				<li>객실 타입 : <span>${reservationCommand.res_bed_type}</span></li>
			</ul>
		...
		<div>
			<h4>객실/패키지</h4>
			<dl>
				<dt>객실명칭<%-- ${reservationCommand.} --%></dt>
				<dd><span id="cost">객실 가격<%-- ${} --%></span></dd>
			</dl>
		</div>
		<div>
			<ul>
