		...
		<tr>
			...
			<td class="line1"><label for="din_class"></label>
				<select id="din_class" name="din_class" style="width:170px; height:25px;" >
						<option value="1"<c:if test="${command.din_class == 1}">selected</c:if>>레스토랑</option>
						<option value="2"<c:if test="${command.din_class == 2}">selected</c:if>>바</option>
						<option value="3"<c:if test="${command.din_class == 3}">selected</c:if>>베이커리</option>
				</select>
			</td>
		</tr>
		<tr>
